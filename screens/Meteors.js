    ...
    if (Object.keys(this.state.meteors).length === 0) {
      ...
      return (
        <View style={styles.container}>
          ...
          <FlatList
            keyExtractor={this.keyExtractor} //keyExtractor - a way for FlatList to differentiate  different elements from each other basically a unique key
            data={meteors} // data - the array whose elements we want to render;
            renderItem={this.renderItem} //renderItem - a function which defines what exactly needs to be rendered for all the data; and
            horizontal={true} //horizontal by default, it’s false but since we want tobuild a carousel effect (which is always horizontal), hence, we kept it as true
          />
        </View>
      );
    }
